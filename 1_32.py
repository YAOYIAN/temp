...
modelname = "deepseek-r1_32b"

with open('g_t_a_dataset.csv', 'r', newline='', encoding='gbk') as csvfile_1:
    with open('result_g_t_a_{}.csv'.format(modelname), 'w', newline='', encoding='gbk') as csvfile:
        ...
        for row in reader:
            ...
            Question: {question}
            Please generate:
            """.format(template = row['e_template'].strip() ,conditions = row['conditions'].strip(),question =  row['Question'].strip())
